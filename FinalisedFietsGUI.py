...
def bluebutton(buttontext, function):
    """Deze functie maakt een button met gegeven vorm, kleur, lettertype etc."""
...
def yellowlabel(labeltext):
    """Deze functie maakt een label met gegeven vorm, kleur, etc."""
...
def display(index):
    """weergeeft de widgets in de que in een rij, in rijnummer index"""
    global que
    for item in que[index]:
        item.grid(row = index, column = que[index].index(item))

def forget(index):
    """alle widgets in que vanaf rij index worden uit de window gehaald"""
    global que
...
def invalidinput(index):
    """Standaard bericht als de gebruiker input niet herkend wordt"""
    global que
    global invalidlabel
...
def invalidpassword(index):
    """Bericht dat weergegeven wordt als de gebruiker herkend word maar het wachtwoord niet"""
    global que
...
def consequencemessage(index):
    """bericht weergegeven wordt als een account geblokkeerd wordt"""
    global que
...
def get_register():
    """geregistreerde gebruikers worden uit de csv gehaald en in de registerlijst gestopt"""
    global registerList
...
def update_register():
    """De csv word aangepast aan de inhoud van de registerlijst"""
    global registerList
...
def maketime():
    """weergeeft de datum en tijd rechts bovenin de window"""
    global kaas
    kaas = True
    while kaas:
        try:
...
def main():
    """Definieerd de window, leest de csv file, en start de window met het menu en tijd"""
    global root
    root = Tk()
...
def register_verify():
    """leest registratieinterface, registreerd nieuwe gebruiker"""
    global registerList
    naam = naamentry.get()
...
def stallen_verify():
    """leest stallen login interface, checkt lege stallen, checkt gebruikersinput, roept stallen() aan"""
    global registerList
    global gebruiker
...
def ophalen_verify():
    """leest ophalen login interface, checkt gebruikersinput, roept ophalen() aan"""
    global registerList
    global gebruiker
    kaas = False
    gebruiker = []
...
def informatie_verify():
    """leest informatie login interface, checkt gebruikerinput, roept informatie aan"""
    global registerList
    global gebruiker
    kaas = False
    gebruiker = []
...
def stallen():
    """geeft stalnummer aan gebruiker, registreerd de fiets in een stal in de csv file"""
    forget(1)
    global registerList
    global gebruiker
    gebruiker[6] = '0'
...
def ophalen():
    """opent de stal, registreerd de fiets als ongestald"""
    forget(1)
    global registerList
    global gebruiker
    gebruiker[6] = '0'
...
def informatie():
    """geeft de gebruiker de stalnummer, staltijd en gestalde tijd van de fiets"""
    forget(1)
    global gebruiker
    gebruiker[6] = '0'
    if gebruiker[5] == '0':
        ...
        staltime = datetime.datetime.strptime(str(gebruiker[5]), "%Y-%m-%d %H:%M:%S.%f")
        stalledtimelabel = str(datetime.datetime.now() - staltime)[0:-10]
        stallnumber = gebruiker[4]
        staltimelabel = yellowlabel('Uw fiets is gestald op: '+str(staltime)[0:16])
        stalledtimelabel = yellowlabel('Uw fiets is '+stalledtimelabel+' gestald')
...
def consequence():
    """wordt aangeroepen als een gebruiker 6 mislukte inlogpogingen waagt"""
    global gebruiker
...
def consequenceloop():
    """deblokkeerd een gebruiker na 5 minuten"""
    global consequencegebruiker
    time.sleep(300)
